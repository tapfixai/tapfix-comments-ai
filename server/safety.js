...
const sexualContextPattern = /\b(lit|bed)\b.{0,48}\b(défaire|defaire|undress|unmake)\b|\b(défaire|defaire|undress|unmake)\b.{0,48}\b(lit|bed)\b/i;
...
const positiveShortPattern = /\b(lovely|beautiful|great|nice|very nice|amazing|wonderful|perfect|cool|i like|really like|love it|love this|muito bom|maravilhos[ao]s?|adorando|adorei|amei|gostei|merci|j[’']adore|j[’']aime|mükemmel)\b/i;
...
export function analyzeComment(comment) {
  ...
  if (sexualPattern.test(comment) || sexualContextPattern.test(comment)) return deletion("sexual", language);
...
function detectLanguage(comment) {
  ...
  if (/[ãõ]/i.test(comment) || /\b(t[oô]\s+adorando|adorando|adorei|amei|gostei|obrigad[ao]|maravilhos[ao]s?|lind[ao]s?|graça|cozinha|bolonhesa|muito bom|relaxante)\b/i.test(text)) {
